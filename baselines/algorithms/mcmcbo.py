...
def run(args):
    ...
    for repeat in range(args.repeat_num):
        ...
        # Save data
            total_eval = len(all_Y[repeat])
            save_len = total_eval // 1000 * 1000
            import os
            results_dir = './baselines/results'
            turbo_dir = os.path.join(results_dir, 'turbo')
            os.makedirs(turbo_dir, exist_ok=True)
            np.save(
                os.path.join(
                    turbo_dir,
                    f"turbo_{args.task}_{dim}_{args.seed}_{args.n_init}_{args.batch_size}_{args.max_evals}_{save_len}.npy"
                ),
                np.array(all_Y[repeat][:save_len])
            )
            ...
            #     results_dir = './baselines/results'
            #     turbo_dir = os.path.join(results_dir, 'turbo')
            #     mcmcbo_dir = os.path.join(results_dir, 'mcmcbo')

            #     os.makedirs(turbo_dir, exist_ok=True)
            #     os.makedirs(mcmcbo_dir, exist_ok=True)

            #     if use_mcmc == False:
            #         np.save(
            #             os.path.join(
            #                 turbo_dir,
            #                 f"turbo_{args.task}_{dim}_{args.seed}_{args.n_init}_{args.batch_size}_{args.max_evals}_{total_eval}.npy"
            #             ),
            #             np.array(Y_turbo[tr_idx].cpu())
            #         )
            #     elif use_mcmc == 'MH':
            #         np.save(
            #             os.path.join(
            #                 mcmcbo_dir,
            #                 f"mcmcbo_{args.task}_{dim}_{args.seed}_{args.n_init}_{args.batch_size}_{args.max_evals}_{total_eval}.npy"
            #             ),
            #             np.array(Y_turbo[tr_idx].cpu())
            #         )
            #     else:
