...
class QFlowTB(nn.Module):
    def __init__(self, x_dim, hidden_dim=512, is_qflow=False, q_net=None, beta=None, dtype = torch.float32):
        super(QFlowTB, self).__init__()
        self.is_qflow = is_qflow
        self.q = q_net
        self.beta = beta
...
class GFN(DiffusionModel):
    def __init__(self, x_dim, diffusion_steps, schedule="linear", predict="epsilon", policy_net="mlp", hidden_dim=512, dtype=torch.float32, reward_net=None, beta=1.0):
        super(GFN, self).__init__(x_dim, diffusion_steps, schedule, predict, policy_net, hidden_dim, dtype)
        self.logZ = torch.nn.Parameter(torch.tensor(0.0, dtype=dtype))
        self.q = reward_net
        self.beta = beta
        self.policy = QFlowTB(x_dim=x_dim, hidden_dim=hidden_dim, dtype=dtype)
        self.policy.q = self.q
        self.policy.beta = beta
    
    def fwd_tb(self, bs, device):
        x, logpf, logpb, log_r = self.get_fwd_traj(bs, device)
        loss = 0.5 * ((logpf + self.logZ - logpb - log_r.detach())**2).mean()
        return loss, x
    
    def bwd_tb(self, x, device):
        x, logpf, logpb, log_r = self.get_bwd_traj(x, device)
        loss = 0.5 * ((logpf + self.logZ - logpb - log_r.detach())**2).mean()
        return loss
        
    def get_fwd_traj(self, bs, device):
        ...
        logpb = torch.zeros_like(logpf)
        
        for i in range(self.diffusion_steps):
            epsilon = self(x, t)
            new_x = self.oneover_sqrta[i] * (
                x - self.mab_over_sqrtmab_inv[i] * epsilon.detach()
                ) + torch.sqrt(self.beta_t[i]) * torch.randn_like(x, dtype=self.dtype)
            #one-step densing distribution
            pf_dist = torch.distributions.Normal(
                self.oneover_sqrta[i] * (x - self.mab_over_sqrtmab_inv[i] * epsilon),
                torch.sqrt(self.beta_t[i]) * torch.ones_like(x, dtype=self.dtype)
            )
            logpf += pf_dist.log_prob(new_x).sum(1)
            #one-step noising distribution
            pb_dist = torch.distributions.Normal(
                torch.sqrt(self.alpha_t[i]) * new_x,
                torch.sqrt(self.beta_t[i]) * torch.ones_like(x, dtype=self.dtype)
            )
        ...
        with torch.no_grad():
            log_r = self.q.log_reward(x, beta=self.beta).squeeze()
        return x.detach(), logpf, logpb, log_r
    
    def get_bwd_traj(self, x, device):
        bs = x.shape[0]
        
        t = torch.zeros((bs, ), device=device, dtype=self.dtype)
        dt = 1 / self.diffusion_steps
        
        logpf = torch.zeros((bs, ), device=device, dtype=self.dtype) 
        logpb = torch.zeros((bs, ), device=device, dtype=self.dtype)
        with torch.no_grad():
            log_r = self.q.log_reward(x, beta=self.beta).squeeze()
        
        for i in range(self.diffusion_steps - 1, -1, -1):
            pb_dist = torch.distributions.Normal(
                torch.sqrt(self.alpha_t[i]) * x,
                torch.sqrt(self.beta_t[i]) * torch.ones_like(x, dtype=self.dtype)
            )
            new_x = pb_dist.sample()
            logpb += pb_dist.log_prob(new_x).sum(1)
            
            epsilon = self(new_x, t + i * dt)
            pf_dist = torch.distributions.Normal(
                self.oneover_sqrta[i] * (new_x - self.mab_over_sqrtmab_inv[i] * epsilon),
                torch.sqrt(self.beta_t[i]) * torch.ones_like(new_x, dtype=self.dtype)
            )
            logpf += pf_dist.log_prob(x).sum(1)
            
            x = new_x
        
        prior_dist = torch.distributions.Normal(torch.zeros_like(x, dtype=self.dtype), torch.ones_like(x, dtype=self.dtype))
        logpf += prior_dist.log_prob(x).sum(1)       
        return x, logpf, logpb, log_r
